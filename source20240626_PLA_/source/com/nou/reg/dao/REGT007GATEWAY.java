...
//                   System.out.print(rs.getString(i));
     ...
     */
     public Vector getRegt007Stut003Syst001Syst002ForUse(Hashtable ht) throws Exception {
		...
		sql.append
		(
			"SELECT A.AYEAR, M.CODE_NAME AS CSMS, NVL(S04.CENTER_CODE,B.CENTER_CODE) as CENTER_CODE, A.TUT_CMPS_CODE, I.CENTER_ABBRNAME AS CCENTER_ABRCODE,A.TUT_CLASS_CODE AS CLASS_CODE, " +
			...
			"A.ASS_CLASS_CODE, H.NAME AS CTCH_IDNO, D.CLSSRM_CODE, A.CRSNO, D.CLASS_KIND, K.CODE_NAME AS CPAYMENT_STATUS, DECODE(L.STNO,'','否','是') AS SCD_YN " +
			"FROM REGT007 A " +
			"	JOIN STUT003 B ON A.STNO=B.STNO " +
			...
			"	JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE " +
			"   JOIN PLAT012 D ON A.AYEAR=D.AYEAR AND A.SMS=D.SMS AND A.CRSNO=D.CRSNO AND D.CENTER_ABRCODE = SUBSTR(A.TUT_CLASS_CODE,1,1) AND D.CMPS_CODE = A.TUT_CMPS_CODE AND D.CLASS_CODE = A.TUT_CLASS_CODE " +
			...
			"	JOIN SYST001 K ON K.KIND = 'PAYMENT_STATUS' AND K.CODE = A.PAYMENT_STATUS " +
			"	LEFT JOIN SCDT004 L ON L.AYEAR = A.AYEAR AND L.SMS = A.SMS AND L.STNO = A.STNO AND L.CRSNO = A.CRSNO " +
			"   JOIN SYST001 M ON M.KIND = 'SMS' AND M.CODE = A.SMS "+
			"WHERE 1=1 "
		);
	...
	//		sql.append("AND A." + (classType.equals("TUT_CMPS_CODE")?"TUT_CLASS_CODE":"LAB_CLASS_CODE") + " = '" + Utility.nullToSpace(ht.get("CLASS_CODE")) + "' ");
		...
		// 班級條件
		sql.append("AND substr(a.tut_class_code,3,1) != 'Z'    ");
		...
		(
			...
			"	JOIN SYST002 I ON I.CENTER_CODE = NVL(S04.CENTER_CODE,b.CENTER_CODE) " +
			"	JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE " +
			"   JOIN PLAT012 D ON A.AYEAR=D.AYEAR AND A.SMS=D.SMS AND D.CENTER_ABRCODE = '0' AND D.CMPS_CODE = 'Z00'  AND A.CRSNO=D.CRSNO  AND D.CLASS_CODE = A.ASS_CLASS_CODE " +
			"	JOIN PLAT002 E ON D.AYEAR=E.AYEAR AND D.SMS=E.SMS AND E.CENTER_ABRCODE = '0'  AND D.CMPS_CODE= 'Z00' " +
			"	JOIN SYST001 F ON D.CLASS_KIND=F.CODE AND F.KIND='CLASS_KIND' " +
			"	JOIN COUT002 G ON A.CRSNO=G.CRSNO " +
			"	LEFT JOIN TRAT001 H ON D.TCH_IDNO=H.IDNO " +
			"   LEFT JOIN PLAT003 J ON J.AYEAR=D.AYEAR AND J.SMS=D.SMS AND J.CENTER_ABRCODE=D.CENTER_ABRCODE AND J.CMPS_CODE=D.CMPS_CODE AND J.CLSSRM_CODE=D.CLSSRM_CODE "+
 	...
 	public Vector getRegt007Plat012ForUse(Hashtable ht) throws Exception {
 		...
 		sql.append("SELECT i.cmps_name as cmps_name_lab,h.cmps_name,h.cmps_code, A.AYEAR,A.SMS,A.STNO,D.NAME AS CSTNO,A.CRSNO,B.CRS_NAME||DECODE(C1.TEACHING_TYPE_NAME,'34','(統一面授)','') AS CCRSNO,A.TUT_CLASS_CODE,A.LAB_CLASS_CODE ");
 		sql.append(",DECODE(A.PAYMENT_STATUS,'1','未繳費','己繳費') AS PAYMENT_STATUS, E.CENTER_ABRCODE, A.MASTER_CLASS_CODE, E.CENTER_NAME, A.TUT_CMPS_CODE, A.LAB_CMPS_CODE, A.ASS_CLASS_CODE,C1.LAB_TIMES,C1.TEACHING_TYPE_NAME,NVL(A.S_CLASS_TYPE,'') AS S_CLASS_TYPE ");
 		...
 		sql.append(", (select count(*) from syst002 where CENTER_ABRCODE = Substr(a.tut_class_code, 0, 1)) as canEdit ");
		...
		sql.append("LEFT JOIN PLAT002 H ON h.AYEAR = a.AYEAR AND h.SMS = a.SMS AND h.CENTER_ABRCODE = substr(a.tut_class_code,1,1) AND A.TUT_CMPS_CODE = H.CMPS_CODE ");
		...
		if (!Utility.nullToSpace(ht.get("PAYMENT_STATUS")).equals("")){
			if("Y".equals((String)ht.get("PAYMENT_STATUS"))){
				sql.append("AND A.PAYMENT_STATUS>'1' ");
			}if("P".equals((String)ht.get("PAYMENT_STATUS"))){
				sql.append("AND A.PAYMENT_STATUS>'1' and (substr(A.TUT_CLASS_CODE,1,1) ='@' or A.TUT_CLASS_CODE is null) ");
...
// 					System.out.print(rs.getString(i));
	 ...
	 * @throws Exception
	 */
	public Vector getRegt007Pla042mForUse(Hashtable ht) throws Exception {
		Vector result = new Vector();

		if (sql.length() > 0)
			sql.delete(0, sql.length());

		sql.append("SELECT i.cmps_name as cmps_name_lab,h.cmps_name,h.cmps_code, A.AYEAR,A.SMS,A.STNO,D.NAME AS CSTNO,A.CRSNO,B.CRS_NAME AS CCRSNO,A.TUT_CLASS_CODE,A.LAB_CLASS_CODE ");
		sql.append(",DECODE(A.PAYMENT_STATUS,'1','未繳費','己繳費') AS PAYMENT_STATUS, E.CENTER_ABRCODE, A.MASTER_CLASS_CODE, E.CENTER_NAME, A.TUT_CMPS_CODE, A.LAB_CMPS_CODE, A.ASS_CLASS_CODE,C1.LAB_TIMES,C1.TEACHING_TYPE_NAME,NVL(A.S_CLASS_TYPE,'') AS S_CLASS_TYPE ");
		sql.append(",NVL((select X.STNO from SCDT004 X where X.AYEAR = A.AYEAR AND X.SMS = A.SMS AND X.CRSNO = A.CRSNO AND X.STNO = A.STNO AND NVL(X.RMK1,' ') NOT LIKE '%退%' ),'') AS SCDT004_STNO ");
		sql.append(", (select count(*) from syst002 where CENTER_ABRCODE = Substr(a.tut_class_code, 0, 1)) as canEdit ");
		sql.append("FROM REGT007 A ");
		//僅呈列統一面授教學(無實體面授)課程
		sql.append("JOIN COUT001 C1 ON C1.AYEAR = A.AYEAR AND C1.SMS = A.SMS AND C1.CRSNO = A.CRSNO AND C1.TEACHING_TYPE_NAME = '34' ");
		sql.append("JOIN COUT002 B ON A.CRSNO=B.CRSNO ");
		sql.append("JOIN STUT003 C ON A.STNO=C.STNO ");
		sql.append("JOIN STUT002 D ON C.IDNO=D.IDNO AND C.BIRTHDATE=D.BIRTHDATE ");
		sql.append("LEFT JOIN STUT004 S04 ON S04.AYEAR = A.AYEAR AND S04.SMS = A.SMS AND S04.STNO = A.STNO ");
		...
		//sql.append("WHERE A.UNQUAL_TAKE_MK='N' AND A.UNTAKECRS_MK='N' AND A.S_CLASS_TYPE IS NULL ");
		sql.append("WHERE A.UNQUAL_TAKE_MK='N' AND A.UNTAKECRS_MK='N' ");
		sql.append("AND A.S_CLASS_TYPE IS NULL  ");
		if (!Utility.nullToSpace(ht.get("PAYMENT_STATUS")).equals("")){
			if("Y".equals((String)ht.get("PAYMENT_STATUS"))){
				sql.append("AND A.PAYMENT_STATUS>'1' ");
			}if("P".equals((String)ht.get("PAYMENT_STATUS"))){
     ...
     */
     public Vector getRegt007Cout001Stut003Scdt008forUse(Hashtable ht) throws Exception {
        ...
        sql.append(
        	...
        	",'' as PLACE_LST1,'' as PLACE_LST2, '' as HAVING_TIMES  "+
        	"FROM SCDT004 S04   "+
        	...
        	//20220112 maggie 111SGU0001 取消排除享有學分學雜費減免者身分學生限制
...
*/
     ...
     /**
      * SGU003M 通知符合申請獎學金(身心障礙_舊制)
      * SGU004M_申請獎學金資料 (HANDICAP_SCHOLAR_NEWOLD = 'OLD')
      * @param ht 條件值
      * @return 回傳 Vector 物件，內容為 Hashtable 的集合，<br>
      *         每一個 Hashtable 其 KEY 為欄位名稱，KEY 的值為欄位的值<br>
      * @throws Exception
     ...
     //空大身障生篩選_舊制         
         sql.append(
             	...
             	",'' as PLACE_LST1,'' as PLACE_LST2, T.HAVING_TIMES   "+
             	"FROM SCDT004 S04   "+
             	"	    JOIN COUT001 CO01 ON S04.AYEAR = CO01.AYEAR AND S04.SMS = CO01.SMS AND S04.CRSNO = CO01.CRSNO   "+
             	"	    JOIN SCDT008 SC08 ON S04.AYEAR = SC08.AYEAR AND S04.SMS = SC08.SMS AND S04.STNO = SC08.STNO  "+
             	"	    JOIN STUT003 ST03 ON ST03.STNO=S04.STNO AND ST03.STTYPE='1' AND SUBSTR(ST03.FTSTUD_ENROLL_AYEARSMS, 1, 3) <= '"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND (  ST03.SPECIAL_STTYPE_TYPE NOT IN ('"+SPECIAL_STTYPE_MK.replaceAll(",", "','")+"') or ST03.SPECIAL_STTYPE_TYPE is null )"+
             	// 身心障礙
             	"	    JOIN SGUT004 SG04 ON SG04.STNO=S04.STNO AND SG04.HAND_NATIVE='2' AND SG04.HANDICAP_EDU_IDENT = '1' AND (SG04.HANDICAP_EDU_IDENT_MK = 'Y' OR SG04.HANDICAP_EDU_IDENT_AYEAR >= '"+Utility.checkNull(ht.get("AYEAR"), "")+"') AND SG04.HANDICAP_SCHOLAR_NEWOLD = 'OLD'  "+
             	// T已領取次數
             	"		LEFT JOIN ( "+
             	"		SELECT 'NOU' AS NOU,M.IDNO, M.BIRTHDATE, SUM(M.TIMES) AS HAVING_TIMES   "+
             	"		   FROM (    "+
             	"		    SELECT '1', STUT003.IDNO, STUT003.BIRTHDATE, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='1' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE = '03' AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
             	"		    UNION    "+
             	"		    SELECT '2', STUT003.IDNO, STUT003.BIRTHDATE, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '1' AND SGUT004.HAND_NATIVE = '2'  AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'OLD' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
             	"		        ) M        "+
             	"		 WHERE 1=1    "+
             	"		 GROUP BY M.IDNO,  M.BIRTHDATE   "+
             	"				   ) T ON T.IDNO = ST03.IDNO AND T.BIRTHDATE = ST03.BIRTHDATE  "+		
             	"WHERE   "+
         ...
         // 空大生同一個身分證字號申領以6(參數PER_RECEIVER_TIMES)次為限 Start... 已領取次數為SGUT003校內取得次數+SGUT004他校取得次數
        if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "").equals("")) {
             sql.append(
          		 "  AND ST03.IDNO NOT IN "+
          		 "  (SELECT M.IDNO "+
				 "   FROM ( "+
				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='1' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE = '03' AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
				 "		UNION "+
				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '1' AND SGUT004.HAND_NATIVE = '2'  AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'OLD' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
				 "        ) M "+		
				 "   WHERE 1=1 "+
				 "   GROUP BY M.IDNO  "+
				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "")+"' " +	     	                          
                 "  ) "
             );
         }

         sql.append("GROUP BY S04.STNO, ST03.ASYS, S04.AYEAR, T.HAVING_TIMES ");
         ...
         // 申請身障助學金的成績上限, 如篩選身障生的助學金時,需排除獎學金的同學,即成績未達申請獎學金的標準,但已達到申請助學金標準<===此段仍為HAVING 函數
     ...
     //空專身障生篩選_舊制
         ...
         sql.append(
              	...
              	"	    JOIN COUT001 CO01 ON S04.AYEAR = CO01.AYEAR AND S04.SMS = CO01.SMS AND S04.CRSNO = CO01.CRSNO   "+
              	"	    JOIN SCDT008 SC08 ON S04.AYEAR = SC08.AYEAR AND S04.SMS = SC08.SMS AND S04.STNO = SC08.STNO  "+
              	"	    JOIN STUT003 ST03 ON ST03.STNO=S04.STNO AND ST03.ASYS='2' AND (  ST03.SPECIAL_STTYPE_TYPE NOT IN ('"+SPECIAL_STTYPE_MK.replaceAll(",", "','")+"') or ST03.SPECIAL_STTYPE_TYPE is null )"+
              	// 身心障礙
              	"	    JOIN SGUT004 SG04 ON SG04.STNO=S04.STNO AND SG04.HAND_NATIVE='2' AND SG04.HANDICAP_EDU_IDENT = '1' AND (SG04.HANDICAP_EDU_IDENT_MK = 'Y' OR SG04.HANDICAP_EDU_IDENT_AYEAR >= '"+Utility.checkNull(ht.get("AYEAR"), "")+"') AND SG04.HANDICAP_SCHOLAR_NEWOLD = 'OLD'  "+
             	// T已領取次數
             	"		LEFT JOIN ( "+
             	"		SELECT 'JC' AS NOU,M.IDNO, M.BIRTHDATE, SUM(M.TIMES) AS HAVING_TIMES   "+
             	"		   FROM (    "+
             	"		    SELECT '1', STUT003.IDNO, STUT003.BIRTHDATE, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='2' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE = '03' AND SGUT003.RE_AUDIT_RESULT = '2'  GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
             	"		    UNION    "+
             	"		    SELECT '2', STUT003.IDNO, STUT003.BIRTHDATE, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '2' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'OLD' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
             	"		        ) M        "+
             	"		 WHERE 1=1    "+
             	"		 GROUP BY M.IDNO,  M.BIRTHDATE   "+
             	"				   ) T ON T.IDNO = ST03.IDNO AND T.BIRTHDATE = ST03.BIRTHDATE  "+		
          ...
          // 空專生同一個身分證字號申領以3(參數PER_RECEIVER_TIMES_J)次為限 Start...
          if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "").equals("")) {
              sql.append(
           		 "  AND ST03.IDNO NOT IN "+
				 "  (SELECT M.IDNO "+
				 "   FROM ( "+
				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS ='2' AND SGUT003.SCHOLAR_TYPE_CODE = '03' AND SGUT003.STNO = STUT003.STNO AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
				 "		UNION "+
				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS ='2' AND SGUT004.HAND_NATIVE = '2'  AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'OLD' AND SGUT004.HANDICAP_EDU_IDENT = '1' AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
				 "        ) M "+		
				 "   WHERE 1=1 "+
				 "   GROUP BY M.IDNO  "+
				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "")+"'	 "+	     	                          
                 "  ) "
              );
          }
          sql.append("GROUP BY S04.STNO, ST03.ASYS, S04.AYEAR, T.HAVING_TIMES ");
       ...
       * @throws Exception
       */
       public Vector getCout001Stut003Sgut004Regt007forUse_New(Hashtable ht) throws Exception {
          Vector result = new Vector();

          if(sql.length() > 0)
              sql.delete(0, sql.length());
          
       //   String SPECIAL_STTYPE_MK = UtilityX.checkEmpty(ht.get("SPECIAL_STTYPE_MK")," "); //不含專班生
      //空大身障生篩選_新制         
          sql.append(
              	"SELECT st03.center_code,S04.STNO, S04.AYEAR, ST03.ASYS, COUNT (CO01.CRSNO) CNTCRS, SUM (CO01.CRD) AS SUMCRS, "+
              	// 取得上一個學年的平均成績(上下學期學期平均)
              	"      (SELECT (A.AVG_MARK+B.AVG_MARK)/2 FROM SCDT008 A, SCDT008 B WHERE A.AYEAR='"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND A.SMS='1' AND B.AYEAR='"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND B.SMS='2' AND A.STNO=B.STNO AND B.STNO=S04.STNO) AS AVG_MARK,  " +
              	// 取得上下個學期的中心(班)排名，(前百分之五十<=50)LST1_PLACE(班排名),LST1(班排名百分比)
              	"(SELECT SC081.AVG_MARK FROM SCDT008 SC081 WHERE SC081.AYEAR = '"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND SC081.SMS = '1' AND SC081.STNO = S04.STNO)|| '-' ||LST1_PLACE||'('||LST1.LST1||'%)' as PLACE_LST1,  "+
              	"(SELECT SC082.AVG_MARK FROM SCDT008 SC082 WHERE SC082.AYEAR = '"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND SC082.SMS = '2' AND SC082.STNO = S04.STNO)|| '-' ||LST2_PLACE||'('||LST2.LST2||'%)' as PLACE_LST2   "+
              	", T.HAVING_TIMES  "+
              	"FROM SCDT004 S04   "+
              	"	    JOIN COUT001 CO01 ON S04.AYEAR = CO01.AYEAR AND S04.SMS = CO01.SMS AND S04.CRSNO = CO01.CRSNO   "+
              	"	    JOIN SCDT008 SC08 ON S04.AYEAR = SC08.AYEAR AND S04.SMS = SC08.SMS AND S04.STNO = SC08.STNO  "+
              	"	    JOIN STUT003 ST03 ON ST03.STNO =  S04.STNO AND ST03.STTYPE= '1' AND SUBSTR(ST03.FTSTUD_ENROLL_AYEARSMS, 1, 3) <= '"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND  ST03.SPECIAL_STTYPE_TYPE is null "+
              	// 身心障礙
              	"	    JOIN SGUT004 SG04 ON SG04.STNO=S04.STNO AND SG04.HAND_NATIVE='2' AND SG04.HANDICAP_EDU_IDENT = '1' AND (SG04.HANDICAP_EDU_IDENT_MK = 'Y' OR SG04.HANDICAP_EDU_IDENT_AYEAR >= '"+Utility.checkNull(ht.get("AYEAR"), "")+"') AND SG04.HANDICAP_SCHOLAR_NEWOLD = 'NEW'  "+
                // 上學期的中心(班)排名，(前百分之五十<=50)
                "		join (select a.stno, A.ROW_NUMBER as lst1_place, ceil(100 * (A.ROW_NUMBER/A.CNT)) AS lst1 "+
                "			from ( "+
                "			SELECT M.ayear, M.sms ,M.STNO, decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) AS CENTER_CODE ,M.AVG_MARK , "+
                "			ROW_NUMBER() over (PARTITION BY decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) ORDER BY M.AVG_MARK DESC ) AS ROW_NUMBER , "+
                "			count(1) over (PARTITION BY decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) ) CNT "+
                "			FROM SCDT008 M  "+ //馬祖列為台北中心
                "			JOIN STUT003 S3 ON S3.STNO = M.STNO AND S3.STTYPE != '2'  AND  S3.SPECIAL_STTYPE_TYPE IS NULL  "+  //班排名排除選修生及專班生
                "			where M.ayear = '" + Utility.checkNull(ht.get("AYEAR2"),"") + "' and M.sms ='1' "+
                "			) A WHERE  1=1 and ceil(100 * (A.ROW_NUMBER/A.CNT)) <= 50 "+ //班排名小於等於50
                "			) lst1 on lst1.STNO = S04.STNO   "+
                // 下學期的中心(班)排名，(前百分之五十<=50)
                "		join (select a.stno, A.ROW_NUMBER as lst2_place,ceil(100 * (A.ROW_NUMBER/A.CNT)) AS lst2 "+
                "			from ( "+
                "			SELECT M.ayear, M.sms ,M.STNO, decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) AS CENTER_CODE ,M.AVG_MARK , "+
                "			ROW_NUMBER() over (PARTITION BY decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) ORDER BY M.AVG_MARK DESC ) AS ROW_NUMBER , "+
                "			count(1) over (PARTITION BY decode(M.CENTER_CODE,'14','02',M.CENTER_CODE) ) CNT "+
                "			FROM SCDT008 M  "+ //馬祖列為台北中心
                "			JOIN STUT003 S3 ON S3.STNO = M.STNO AND S3.STTYPE != '2'  AND  S3.SPECIAL_STTYPE_TYPE IS NULL  "+  //班排名排除選修生及專班生
                "			where M.ayear = '" + Utility.checkNull(ht.get("AYEAR2"),"") + "' and M.sms ='2' "+
                "			) A WHERE  1=1 and ceil(100 * (A.ROW_NUMBER/A.CNT)) <= 50 "+ //班排名列前面50%，即小於等於50
                "			) lst2 on lst2.STNO = S04.STNO   "+
	             // T已領取次數
	          	"		LEFT JOIN ( "+
	          	"		SELECT 'NOU' AS NOU,M.IDNO, M.BIRTHDATE, SUM(M.TIMES) AS HAVING_TIMES   "+
	          	"		   FROM (    "+
	          	"		    SELECT '1', STUT003.IDNO, STUT003.BIRTHDATE, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='1' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE IN ('06','07') AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
	          	"		    UNION    "+
	          	"		    SELECT '2', STUT003.IDNO, STUT003.BIRTHDATE, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '1' AND SGUT004.HAND_NATIVE = '2'  AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
	          	"		        ) M        "+
	          	"		 WHERE 1=1    "+
	          	"		 GROUP BY M.IDNO,  M.BIRTHDATE   "+
	          	"				   ) T ON T.IDNO = ST03.IDNO AND T.BIRTHDATE = ST03.BIRTHDATE  "+	
          ...
          // 空大生同一個身分證字號申領以6(參數PER_RECEIVER_TIMES)次為限 Start...
          if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "").equals("")) {
        	  sql.append(
	       		 "  AND ST03.IDNO NOT IN "+
	       		 "  (SELECT M.IDNO "+
				 "   FROM ( "+
				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.STTYPE='1' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE in ('06','07') AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
				 "		UNION "+
				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.STTYPE = '1' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
				 "        ) M "+		
				 "   WHERE 1=1 "+
				 "   GROUP BY M.IDNO  "+
				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "")+"' " +	     	                          
          ...
          sql.append("GROUP BY ST03.CENTER_CODE, S04.STNO, ST03.ASYS, S04.AYEAR,LST1_PLACE, LST1.LST1, LST2_PLACE,LST2.LST2, T.HAVING_TIMES  ");
          sql.append("HAVING COUNT (CO01.CRSNO) >= '"+ Utility.checkNull(ht.get("MIN_PT_CRS_CNT"),"") +"' " +
          				"and SUM (CO01.CRD) >= '"+ Utility.nullToSpace(ht.get("MIN_PT_CRD_CNT"))+"' "+
          				// 申請身障獎助學金的成績下限
          				"and (SELECT (A.AVG_MARK+B.AVG_MARK)/2 FROM SCDT008 A, SCDT008 B WHERE A.AYEAR='"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND A.SMS='1' AND B.AYEAR='"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND B.SMS='2' AND A.STNO=B.STNO AND B.STNO=S04.STNO) >=  '" + Utility.checkNull(ht.get("MIN_MARK_AVG"),"") + "' "
      ...
      //空專身障生篩選_新制
          ...
          sql.append(
              	...
              	"FROM SCDT004 S04   "+
               	"	    JOIN COUT001 CO01 ON S04.AYEAR = CO01.AYEAR AND S04.SMS = CO01.SMS AND S04.CRSNO = CO01.CRSNO   "+
               	"	    JOIN SCDT008 SC08 ON S04.AYEAR = SC08.AYEAR AND S04.SMS = SC08.SMS AND S04.STNO = SC08.STNO  "+
               	"	    JOIN STUT003 ST03 ON ST03.STNO=S04.STNO AND ST03.ASYS='2' AND ST03.SPECIAL_STTYPE_TYPE is null "+
               	// 身心障礙
               	"	    JOIN SGUT004 SG04 ON SG04.STNO=S04.STNO AND SG04.HAND_NATIVE='2' AND SG04.HANDICAP_EDU_IDENT = '1' AND (SG04.HANDICAP_EDU_IDENT_MK = 'Y' OR SG04.HANDICAP_EDU_IDENT_AYEAR >= '"+Utility.checkNull(ht.get("AYEAR"), "")+"') AND SG04.HANDICAP_SCHOLAR_NEWOLD = 'NEW'  "+
                // 上學期的中心(班)排名，(前百分之五十<=50)
                "		join (select a.stno, A.ROW_NUMBER as lst1_place, ceil(100 * (A.ROW_NUMBER/A.CNT)) AS lst1 "+
                ...
                "			) lst2 on lst2.STNO = S04.STNO   "+  
	             // T已領取次數
	          	...
	          	"		SELECT 'NOU' AS NOU,M.IDNO, M.BIRTHDATE, SUM(M.TIMES) AS HAVING_TIMES   "+
	          	"		   FROM (    "+
	          	"		    SELECT '1', STUT003.IDNO, STUT003.BIRTHDATE, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='2' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE IN ('06','07') AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
	          	"		    UNION    "+
	          	"		    SELECT '2', STUT003.IDNO, STUT003.BIRTHDATE, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '2' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO , STUT003.BIRTHDATE   "+
	          	"		        ) M        "+
	          	"		 WHERE 1=1    "+
	          	"		 GROUP BY M.IDNO,  M.BIRTHDATE   "+
	          	"				   ) T ON T.IDNO = ST03.IDNO AND T.BIRTHDATE = ST03.BIRTHDATE  "+	
           ...
           if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "").equals("")) {
        	   sql.append(
             	 "  AND ST03.IDNO NOT IN "+
  				 "  (SELECT M.IDNO "+
  				 "   FROM ( "+
  				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.STTYPE='3' AND SGUT003.SCHOLAR_TYPE_CODE in ('06','07') AND SGUT003.STNO = STUT003.STNO AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
  				 "		UNION "+
  				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.STTYPE = '3' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1' AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
  				 "        ) M "+		
  				 "   WHERE 1=1 "+
  				 "   GROUP BY M.IDNO  "+
  				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "")+"'	 "+	     	                          
        ...
        * @throws Exception
        */
        public Vector getCout001Stut003Sgut004Regt007forUse_new2(Hashtable ht) throws Exception {
           Vector result = new Vector();
       ...
       //空大新制身心障礙_補助金         
           sql.append(
               	...
               	"	    JOIN COUT001 CO01 ON S04.AYEAR = CO01.AYEAR AND S04.SMS = CO01.SMS AND S04.CRSNO = CO01.CRSNO   "+
               	"	    JOIN SCDT008 SC08 ON S04.AYEAR = SC08.AYEAR AND S04.SMS = SC08.SMS AND S04.STNO = SC08.STNO  "+
               	"	    JOIN STUT003 ST03 ON ST03.STNO=S04.STNO AND ST03.ASYS='1' AND SUBSTR(ST03.FTSTUD_ENROLL_AYEARSMS, 1, 3) <= '"+Utility.checkNull(ht.get("AYEAR2"), "")+"' AND (  ST03.SPECIAL_STTYPE_TYPE NOT IN ('"+SPECIAL_STTYPE_MK.replaceAll(",", "','")+"') or ST03.SPECIAL_STTYPE_TYPE is null )"+
               	// 身心障礙
               	"	    JOIN SGUT004 SG04 ON SG04.STNO=S04.STNO AND SG04.HAND_NATIVE='2' AND SG04.HANDICAP_EDU_IDENT = '1' AND (SG04.HANDICAP_EDU_IDENT_MK = 'Y' OR SG04.HANDICAP_EDU_IDENT_AYEAR >= '"+Utility.checkNull(ht.get("AYEAR"), "")+"') AND SG04.HANDICAP_SCHOLAR_NEWOLD = 'NEW'  "+
	             // T已領取次數
	          	"		LEFT JOIN ( "+
	          	...
	          	"				   ) T ON T.IDNO = ST03.IDNO AND T.BIRTHDATE = ST03.BIRTHDATE  "+	               	
               	...
               	"	    AND S04.AYEAR = '" + Utility.checkNull(ht.get("AYEAR1"),"") + "' "+
               	"	    AND ST03.CENTER_CODE LIKE '" + Utility.checkNull(ht.get("CENTER_CODE"),"") + "%'  "+
               	//不存在已申請新制身障獎補助金及已篩選新制獎學金
               	"  		AND ST03.STNO NOT IN (SELECT SG03.STNO FROM SGUT003 SG03 WHERE SG03.AYEAR = '" + Utility.checkNull(ht.get("AYEAR"),"") + "' AND SG03.SCHOLAR_TYPE_CODE IN ('06','07') UNION SELECT SG29.STNO FROM SGUT029 SG29 WHERE SG29.AYEAR = '" + Utility.checkNull(ht.get("AYEAR"),"") + "' AND SG29.QUAL_ITEM = '06')"
               );
          ...
          if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "").equals("")) {
               sql.append(
            		 ...
            		 "  (SELECT M.IDNO "+
  				 "   FROM ( "+
  				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='1' AND SGUT003.STNO = STUT003.STNO AND SGUT003.SCHOLAR_TYPE_CODE in ('06','07') AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
  				 "		UNION "+
  				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS = '1' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1'  AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
  				 "        ) M "+		
  				 "   WHERE 1=1 "+
  				 "   GROUP BY M.IDNO  "+
  				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES"), "")+"' " +	     	                          
           ...
           sql.append(
            ...
            if(!Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "").equals("")) {
                sql.append(
  				 ...
  				 "  (SELECT M.IDNO "+
  				 "   FROM ( "+
  				 "		SELECT '1', STUT003.IDNO, nvl(COUNT(1),0) AS TIMES FROM SGUT003, STUT003 WHERE STUT003.ASYS='2' AND SGUT003.SCHOLAR_TYPE_CODE in ('06','07') AND SGUT003.STNO = STUT003.STNO AND SGUT003.RE_AUDIT_RESULT = '2' GROUP BY STUT003.IDNO "+
  				 "		UNION "+
  				 "		SELECT '2', STUT003.IDNO, nvl(SUM(SGUT004.OTHER_SHCOOL_TIMES),0) AS TIMES FROM SGUT004, STUT003 WHERE STUT003.ASYS='2' AND SGUT004.HAND_NATIVE = '2' AND SGUT004.HANDICAP_SCHOLAR_NEWOLD = 'NEW' AND SGUT004.HANDICAP_EDU_IDENT = '1' AND SGUT004.STNO = STUT003.STNO GROUP BY STUT003.IDNO "+
  				 "        ) M "+		
  				 "   WHERE 1=1 "+
  				 "   GROUP BY M.IDNO  "+
  				 "   HAVING SUM(M.TIMES) >= '"+Utility.checkNull(ht.get("PER_RECEIVER_TIMES_J"), "")+"'	 "+	     	                          
...
//                 rs2.close();
     ...
     */
     public Vector getRegt007Stut003Stut002Cout001Print(Hashtable ht) throws Exception {
        ...
        " FROM REGT007 A "+
        " JOIN COUT001 E ON E.CRSNO=A.CRSNO AND E.AYEAR=A.AYEAR AND E.SMS=A.SMS "+
...
//AND C.VOCATION='02'
             ...
             String FACULTY_CODE        =  Utility.checkNull(ht.get("FACULTY_CODE"),"");          //開課學系
             ...
             if(!"".equals(FACULTY_CODE))
                 sql.append(" AND E.FACULTY_CODE = '"+FACULTY_CODE+ "'");
             if(!"".equals(CRSNO))
                sql.append(" AND A.CRSNO = '"+CRSNO+ "'");
             if(!"".equals(VOCATION))
                sql.append(" AND C.VOCATION = '"+VOCATION+ "'");
             if(!"".equals(EDUBKGRD_GRADE))
                sql.append(" AND B.EDUBKGRD_GRADE = '"+EDUBKGRD_GRADE+ "'");
             if(!"".equals(CENTER_CODE))
     ...
     */
     public Vector getRegt007Reg134rSTNOPrint(Hashtable ht) throws Exception {
         ...
         sql.append(        	
	        " SELECT UNIQUE A.STNO,C.NAME||DECODE(R5.PAYMENT_STATUS,'1','*','') AS NAME,D.CODE_NAME||decode(substr(b.idno,2,1),'A',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'B',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'C',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'D',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'')  AS STTYPE "+
	        " FROM REGT007 A "+
	        " JOIN STUT003 B ON A.STNO=B.STNO "+
	        " JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE "+
	        " JOIN SYST001 D ON B.STTYPE=D.CODE AND D.KIND='STTYPE' "+
	        " JOIN REGT005 R5 ON R5.AYEAR =A.AYEAR AND R5.SMS =A.SMS AND R5.STNO=A.STNO "+
	        " WHERE 1=1 "+
    	 ...
    	 }else if("3".equals(PRT_MANNER)){
        	 sql.append(        	
			" SELECT UNIQUE A.STNO,C.NAME||DECODE(R5.PAYMENT_STATUS,'1','*','')||DECODE(B.CENTER_CODE,'" + CENTER_CODE + "','',' ('||E.CENTER_ABBRNAME||')') as NAME,D.CODE_NAME||decode(substr(b.idno,2,1),'A',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'B',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'C',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'D',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'')  AS STTYPE "+
        	" FROM REGT007 A "+
	        " JOIN STUT003 B ON A.STNO=B.STNO "+
	        " JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE "+
	        " JOIN SYST001 D ON B.STTYPE=D.CODE AND D.KIND='STTYPE' "+
	        " JOIN SYST002 E ON B.CENTER_CODE=E.CENTER_CODE  "+
	        " JOIN REGT005 R5 ON R5.AYEAR =A.AYEAR AND R5.SMS =A.SMS AND R5.STNO=A.STNO "+
	        " WHERE 1=1 "+
	        " AND SUBSTR(A.TUT_CLASS_CODE,1,1) = (SELECT R1.CENTER_ABRCODE FROM SYST002 R1 WHERE R1.CENTER_CODE = '" + CENTER_CODE + "') "+
	        " AND A.UNTAKECRS_MK='N' AND A.UNQUAL_TAKE_MK='N' "
          );
    	 }else if("4".equals(PRT_MANNER)){
        	 sql.append(        	
        	" SELECT UNIQUE A.STNO,C.NAME||DECODE(R5.PAYMENT_STATUS,'1','*','')||DECODE(E.CENTER_CODE,'" + CENTER_CODE + "','',' ('||F.CENTER_ABBRNAME||')') as NAME,D.CODE_NAME||decode(substr(b.idno,2,1),'A',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'B',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'C',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'D',substr(b.idno,1,2)||'-'||C.RESIDENCE_DATE||'-'||C.MOBILE,'')  AS STTYPE "+
        	" FROM REGT007 A "+
            " JOIN STUT003 B ON A.STNO=B.STNO "+
        	" JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE "+
            " JOIN STUT004 E ON A.AYEAR = E.AYEAR AND A.SMS = E.SMS AND A.STNO=E.STNO "						+
        	" JOIN SYST001 D ON B.STTYPE=D.CODE AND D.KIND='STTYPE' "+
        	" JOIN SYST002 F ON E.CENTER_CODE=F.CENTER_CODE  "+
        	" JOIN REGT005 R5 ON R5.AYEAR =A.AYEAR AND R5.SMS =A.SMS AND R5.STNO=A.STNO "+
            " WHERE 1=1 "+
            " AND SUBSTR(A.TUT_CLASS_CODE,1,1) = (SELECT R1.CENTER_ABRCODE FROM SYST002 R1 WHERE R1.CENTER_CODE = '" + CENTER_CODE + "') "+
            " AND A.UNTAKECRS_MK='N' AND A.UNQUAL_TAKE_MK='N' "
         ...
         if(!"".equals(CENTER_CODE)){
        	 if("1".equals(PRT_MANNER)){
        		 ...
        		 sql.append(" AND B.CENTER_CODE IN ('01','02','03','04','05','06','07','08','09','10','11','12','13','14','15') ");
        	 }else if("4".equals(PRT_MANNER)){
        		 sql.append(" AND E.CENTER_CODE IN ('01','02','03','04','05','06','07','08','09','10','11','12','13','14','15') ");
        	 } 
         }
         if(!"".equals(CMPS_CODE))
     ...
     */
     public Vector getRegt007Reg134rCRSNOPrint(Hashtable ht) throws Exception {
        ...
        sql.append("SELECT  distinct A.CRSNO,B.CRS_NAME,A.TUT_CMPS_CODE,A.TUT_CLASS_CODE AS MASTER_CLASS_CODE,A.ASS_CLASS_CODE ,TO_CHAR(NVL(B.CRD,'0')) AS CRD,");
        sql.append("'1' AS RMK ");
        ...
        sql.append("left JOIN SYST002 S2 ON SUBSTR(A.TUT_CLASS_CODE,1,1) = S2.CENTER_ABRCODE  ");
     ...
     */
     public DBResult getRegt007REG126Print(Hashtable ht) throws Exception {
        ...
        if(sql.length() > 0) {
            ...
            //計算此科目此職業人數
       sql.append(
        " SELECT B.STTYPE,C.SEX,C.BIRTHDATE, F.ENROLL_BASEDATE "+
        " FROM REGT005 R "+
        ...
        " JOIN STUT002 C ON B.IDNO=C.IDNO AND B.BIRTHDATE=C.BIRTHDATE "+
        " JOIN SYST002 D ON B.CENTER_CODE=D.CENTER_CODE "+
        " JOIN SYST005 F ON F.AYEAR = R.AYEAR AND F.SMS = R.SMS "+
        " WHERE 0=0 "+
        " AND R.TAKE_ABNDN ='N'  "+
        " AND A.UNQUAL_TAKE_MK ='N'  "+
        " AND A.UNTAKECRS_MK ='N' "
	...
	public void getAutIndexSResult1(Vector vt, Hashtable ht) throws Exception {
		...
		try
		{
			...
			(
				...
				"b.cmps_code, e.cmps_name||'-'||e1.cmps_name as cmps_name, b.class_kind, g.code_name, b.clssrm_code, decode(b.CLASS_KIND ,'7','0',f.crd) as crd, DECODE(P28.TCH_IDNO_YN,'Y',h.NAME,'') AS NAME, SUBSTR(i.faculty_code,1,1) AS FACULTY_CODE, i.CRS_GROUP_CODE, " +
			...
			//	"left join plat012 b on a.ayear = b.ayear AND a.sms = b.sms AND a.crsno = b.crsno AND a.tut_cmps_code=b.cmps_code AND a.ASS_class_code = b.class_code " +
				"left join plat012 b on a.ayear = b.ayear AND a.sms = b.sms AND a.crsno = b.crsno AND a.tut_class_code = b.class_code " +
 ...
 // 取得某班的學生資料-->用來寄發選取某個學生
    public Vector getReg024mQuery(Hashtable ht) throws Exception {
		...
		try
		{
			...
			sql.append("r7.ASS_CLASS_CODE||(select t01.name||t01.nou_email from plat012 p12 join trat001 t01 on p12.tch_idno = t01.idno where p12.ayear = r7.ayear and p12.sms =r7.sms and p12.crsno = r7.crsno and p12.class_code = r7.ass_class_code and p12.tch_idno is not null) as ASS_CLASS_CODE,  ");
			sql.append("EXAM_SEAT, LAB_SEAT, TUT_SEAT, VOLNUTEER_TIME01, VOLNUTEER_TIME02, VOLNUTEER_TIME03, VOLNUTEER_TIME04, VOLNUTEER_TIME05, VOLNUTEER_TIME06, VOLNUTEER_TIME07, VOLNUTEER_TIME08, VOLNUTEER_TIME09, VOLNUTEER_TIME10, S_CLASS_TYPE,");
			sql.append("(select CMPS_NAME from plat002 a where a.ayear =r7.ayear and a.sms =r7.sms and substr(r7.TUT_class_code,1,1) = a.center_abrcode  and r7.TUT_CMPS_CODE = a.CMPS_CODE) AS TUT_CMPS_NAME,");
            sql.append("(select CMPS_NAME from plat002 a where a.ayear =r7.ayear and a.sms =r7.sms and substr(r7.LAB_class_code,1,1) = a.center_abrcode  and r7.LAB_CMPS_CODE = a.CMPS_CODE) AS LAB_CMPS_NAME,");
            sql.append("(select CMPS_NAME from plat002 a where a.ayear =r7.ayear and a.sms =r7.sms and substr(r7.tut_class_code,1,1) = a.center_abrcode and r7.ALLOCATE_CMPS_CODE = a.CMPS_CODE ) AS ALLOCATE_CMPS_NAME ");
            sql.append(", DECODE(R7.STU_TEACHING_TYPE,'',decode(r7.master_class_code,'@0@0','視訊夜間班', '@0@1','視訊下午班', '@0@2','視訊上午班','實體面授班'), '@0@1','視訊下午班', '@0@2','視訊上午班','1','實體面授班' ) as STU_TEACHING_TYPE ");
            sql.append("from regt005 r5 ");
            sql.append("join regt007 r7 on r5.ayear =r7.ayear and r5.sms =r7.sms and r5.stno =r7.stno ");
            sql.append("join stut003 s3 on r7.stno = s3.stno ");
 ...
 // 取得某班的學生資料-->用來寄發選取某個學生
		...
		try
		{
			...
			sql.append(
				...
				"WHERE "+
					...
					"AND A.UNQUAL_TAKE_MK='N' "+
					"AND A.UNTAKECRS_MK='N' "+
					"AND nvl(substr(a.MASTER_CLASS_CODE,3,1),0) not in ('@','Z') "+ 
					"AND A.S_CLASS_TYPE IS NULL "
			);
			...
			PreparedStatement ps = conn.prepareStatement
			(
					"UPDATE REGT007 SET MASTER_CLASS_CODE=?, TUT_CLASS_CODE=?, ASS_CLASS_CODE=?, "+
			...
			while (rs.next())
			{
				...
				ps.setString(4, rs.getString("CRSNO"));
				ps.setString(5, rs.getString("STNO")); // 學號
    ...
    public String getLabStuNumForPcs110r(Hashtable ht) throws Exception {
        ...
        if(!Utility.nullToSpace(ht.get("PRT_MANNER")).equals("")) {
            ...
            //1 繳費前 2繳費後
            if("1".equals(PRT_MANNER)){
            	condition ="AND A.unqual_take_mk='N' AND A.UNTAKECRS_MK = 'N' \n";
            }
            if("2".equals(PRT_MANNER)){
            	condition ="AND A.unqual_take_mk='N' AND A.UNTAKECRS_MK = 'N' \n";
    ...
    public String getLabTotalPayableAmtForPcs110r(Hashtable ht) throws Exception {
        ...
        if(!Utility.nullToSpace(ht.get("PRT_MANNER")).equals("")) {
            ...
            //1 繳費前 2繳費後 3出納對帳用
            if("1".equals(PRT_MANNER)){
            	condition ="AND A.TAKE_ABNDN='N' AND NVL(SUBSTR(B.WRITEOFF_NO,7),0)=(select MAX(NVL(SUBSTR(R10.WRITEOFF_NO,7),0)) FROM REGT010 R10 WHERE R10.AYEAR =B.AYEAR AND R10.SMS =B.SMS AND R10.STNO =B.STNO AND R10.ABNDN_ORDER = 'N' ) \n";
            }
            if("2".equals(PRT_MANNER)){
            	condition ="AND A.TAKE_ABNDN='N' AND A.PAYMENT_STATUS IN ('2','3','4') AND (NVL(B.WRITEOFF_NO,0))= (select DISTINCT MAX(NVL(R10.WRITEOFF_NO,0)) FROM REGT010 R10 WHERE R10.AYEAR =B.AYEAR AND R10.SMS =B.SMS AND R10.STNO =B.STNO AND R10.ABNDN_ORDER = 'N' AND B.PAYMENT_STATUS != '1') \n";         	
            }
            if("3".equals(PRT_MANNER)){
            	condition ="AND A.PAYMENT_STATUS IN ('2','3','4') AND NVL(SUBSTR(B.WRITEOFF_NO,7),0)=(select MAX(NVL(SUBSTR(R10.WRITEOFF_NO,7),0)) FROM REGT010 R10 WHERE R10.AYEAR =B.AYEAR AND R10.SMS =B.SMS AND R10.STNO =B.STNO AND SUBSTR(R10.WRITEOFF_NO,1,4) != '5293' AND R10.ABNDN_ORDER = 'N' ) \n";
            }
        }
        String sql =
        	"SELECT NVL(SUM(LAB_FEE),'0') AS TOTAL_LAB_FEE " +
        	"FROM REGT005 A " +
        	"JOIN REGT010 B ON A.AYEAR=B.AYEAR AND A.SMS=B.SMS AND A.STNO=B.STNO AND B.ABNDN_ORDER = 'N' " +        			
        	"WHERE " +
        		"A.AYEAR = '"+ht.get("AYEAR")+"' " +
        		"AND A.SMS = '"+ht.get("SMS")+"' " +
        		"AND B.ASYS ='"+ht.get("ASYS")+"' " +
        			condition+
        		" ";
    ...
    public Vector getStuChoiceCrs(String ayear, String sms, String stno) throws Exception {
        ...
        String sql =
        	"SELECT A.STNO, A.CRSNO, B.CRS_NAME, B.CRS_NAME||'('||B.CRD||'學分)' AS CRS_NAME_CRD, " +
        ...
        try {
            ...
            UtilityX.combinCompareNextTheSameData(pk, "CRS_NAME_CRD", ",", tmp, result);
        } catch (Exception e) {
            throw e;
        } finally {
            if(rs != null)
    ...
    public Vector getReg107rCrsno1(Hashtable ht) throws Exception {
        ...
        sql.append("select unique f.crsno, h.crs_name,h.crd ");
        sql.append("FROM REGT005 A  ");
        sql.append("JOIN REGT010 B ON A.AYEAR=B.AYEAR AND A.SMS=B.SMS AND A.STNO=B.STNO   ");        
        sql.append("join regt011 f on f.ayear = b.ayear and f.sms = b.sms and f.writeoff_no = b.writeoff_no ");        
        sql.append("join cout002 h on h.crsno = f.crsno ");
        sql.append("WHERE 1=1  ");
        sql.append("AND A.AYEAR = '"+Utility.nullToSpace(ht.get("AYEAR"))+"' AND A.SMS = '"+Utility.nullToSpace(ht.get("SMS"))+"'  ");
        sql.append("AND NVL(A.PAYMENT_STATUS,'1') != '1'  ");
        sql.append("AND NVL(SUBSTR(B.WRITEOFF_NO,7),0) = (SELECT MAX(NVL(SUBSTR(R10.WRITEOFF_NO,7),0)) FROM REGT010 R10 WHERE R10.AYEAR=B.AYEAR AND R10.SMS=B.SMS AND R10.STNO=B.STNO  AND SUBSTR(R10.WRITEOFF_NO,1,4) != '5293' AND R10.ABNDN_ORDER='N')  ");        
        sql.append("order by f.crsno ");
   ...
   public Vector getReg189rCrsno(Hashtable ht) throws Exception {
        ...
        sql.append(
        		...
        		"FROM REGT007 A " +
        		"JOIN COUT002 B ON A.CRSNO=B.CRSNO "+
        		"join plat002 e on e.ayear = a.ayear and e.sms = a.sms and e.center_abrcode = substr(a.tut_class_code,1,1) and e.cmps_code = a.tut_cmps_code "+
        		"WHERE 1=1 "
        );
        if(!Utility.nullToSpace(ht.get("AYEAR")).equals("")) {
            sql.append("AND A.AYEAR = '" + Utility.nullToSpace(ht.get("AYEAR")) + "' ");
 ...
 // 清空學生面授班級資訊
    ...
    public Hashtable getDataForReg179r(Hashtable ht) throws Exception {
        ...
        if(sql.length() > 0) {
        	...
        	String condition1 = " ('"+Utility.nullToSpace(ht.get("AYEAR"))+"','1'),"+
 				   " ('"+Utility.nullToSpace(ht.get("AYEAR"))+"','2'),"+
 				   " ('"+Utility.nullToSpace(ht.get("AYEAR"))+"','3') ";
        ...
        sql.append(
	        ...
	        if(Utility.nullToSpace(ht.get("SMS")).equals("")) {
	        	sql.append("AND (A.AYEAR,A.SMS) IN ("+condition+")   \n");
	        } else {
	        	sql.append("AND (A.AYEAR,A.SMS) IN ("+condition1+")   \n");
	        }
		sql.append(
			"AND A.UNQUAL_TAKE_MK = 'N' \n"+
			"AND A.UNTAKECRS_MK = 'N' \n"+
			...
			"              AND D.WRITEOFF_NO = (SELECT MAX(R10.WRITEOFF_NO) \n"+
			"                                             FROM REGT010 R10 \n"+
    ...
    /**
 	...
 	public Vector getPla037MQuery(Hashtable ht) throws Exception {
 		...
 		sql.append(",NVL((select X.STNO from SCDT004 X where X.AYEAR = M.AYEAR AND X.SMS = M.SMS AND X.CRSNO = M.CRSNO AND X.STNO = M.STNO AND NVL(X.RMK1,' ') NOT LIKE '%退%' ),'') AS SCDT004_STNO ");
 	...
 	/**
 	 *  取得某一學年期未編班的的學生資料,一個學生一科一筆資料<====編班的部份	(//2020/08/27 從 getQueryNonPlaForPla032m_2 修改過來的 因怕原本的還會用到 先保留)
 	 */
 	public Vector getQueryNonPlaForPla032m_3(Hashtable ht) throws Exception {
 		Vector result = new Vector();
 		...
 		try{
 			String sql = 
		    			"\n"+
		    			"SELECT B.AYEAR, B.SMS, B.STNO,E.NAME,B.CRSNO,C.CRS_NAME, \n"+
		            	"DECODE(B.STU_TEACHING_TYPE,'',(decode(B.MASTER_CLASS_CODE,'','實體面授班','1','實體面授班',B.MASTER_CLASS_CODE)),'1','實體面授班',B.STU_TEACHING_TYPE) AS STU_TEACHING_TYPE, \n"+
		    			"decode(B.TUT_CLASS_CODE,'1','實體面授不開班',B.TUT_CLASS_CODE) AS CLASS_CODE, B.LAB_CLASS_CODE, B.ASS_CLASS_CODE,  \n"+
		            	"F.CENTER_ABBRNAME, H.CMPS_NAME,  \n"+
		            	"F.CENTER_ABRCODE, B.TUT_CMPS_CODE AS CMPS_CODE, \n"+
		            	"CASE WHEN (B.TUT_CLASS_CODE IS NULL AND B.ASS_CLASS_CODE IS NULL ) THEN '尚未編班' \n"+
		            	"     WHEN (B.TUT_CLASS_CODE IS NOT NULL AND B.ASS_CLASS_CODE IS NULL) THEN '編班異常無上課作業班' \n"+
		            	"     WHEN (SUBSTR(B.ASS_CLASS_CODE,1,3) = 'ZZZ' AND (SUBSTR(B.ASS_CLASS_CODE,4,3) != SUBSTR(B.TUT_CLASS_CODE,4,3))) THEN '網路班考試班與上課作業班不一致' \n"+
		            	"     WHEN ((SUBSTR(B.ASS_CLASS_CODE,1,3) != 'ZZZ' AND B.LAB_CLASS_CODE IS NULL) AND (B.ASS_CLASS_CODE != B.TUT_CLASS_CODE)) THEN '考試班與上課作業班不一致'  \n"+
		            	"     ELSE '其他請確認' END AS PLACHECK \n"+
		            	"FROM REGT007 B \n"+
		            	"JOIN COUT002 C ON C.CRSNO=B.CRSNO \n"+
		            	"JOIN STUT003 D ON D.STNO=B.STNO \n"+
		            	"JOIN STUT002 E ON E.IDNO=D.IDNO AND E.BIRTHDATE=D.BIRTHDATE \n"+
		            	"JOIN STUT004 I ON I.AYEAR = B.AYEAR AND I.SMS = B.SMS AND I.STNO = B.STNO  \n"+
		            	"JOIN SYST002 F ON F.CENTER_CODE=I.CENTER_CODE \n" +
		            	"LEFT JOIN PLAT002 H ON B.AYEAR = H.AYEAR AND B.SMS = H.SMS AND B.TUT_CMPS_CODE = H.CMPS_CODE AND F.CENTER_ABRCODE = H.CENTER_ABRCODE \n" +
		            	"WHERE \n" +
		            	"B.AYEAR='"+ht.get("AYEAR")+"' \n" +
		            	"AND B.SMS='"+ht.get("SMS")+"' \n"+
		            	...
		            	"AND B.PAYMENT_STATUS != '1' \n" +
		            	"AND ( \n" +
		            	"    (B.TUT_CLASS_CODE IS NULL AND B.ASS_CLASS_CODE IS NULL ) \n" +
		            	" OR (B.TUT_CLASS_CODE IS NOT NULL AND B.ASS_CLASS_CODE IS NULL) \n" +
		            	" OR (SUBSTR(ASS_CLASS_CODE,1,3) = 'ZZZ' AND (SUBSTR(ASS_CLASS_CODE,4,3) != SUBSTR(TUT_CLASS_CODE,4,3))) \n" +
		            	" OR ((SUBSTR(B.ASS_CLASS_CODE,1,3) != 'ZZZ' AND B.LAB_CLASS_CODE IS NULL) AND (B.ASS_CLASS_CODE != B.TUT_CLASS_CODE)) \n" +
		            	" OR ((LENGTH(B.TUT_CLASS_CODE)<5 OR B.TUT_CLASS_CODE IS NULL) or ((LENGTH(B.TUT_CLASS_CODE)= 6 AND (SUBSTR(B.ASS_CLASS_CODE,1,3) != 'ZZZ' AND B.S_CLASS_TYPE IS NULL) and substr(b.tut_class_code, 1,1) != '" + ht.get("CENTER_ABRCODE").toString() + "' ))) \n"+
 						"    ) \n"; 
		 			if(!"".equals(ht.get("CENTER_ABRCODE"))) {
		    			sql += "AND I.CENTER_CODE = (SELECT D1.CENTER_CODE FROM SYST002 D1 WHERE D1.CENTER_ABRCODE = '" + ht.get("CENTER_ABRCODE").toString() + "') ";
		    		}
